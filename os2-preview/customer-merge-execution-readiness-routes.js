...
function logFailure(error,authorisationId){console.error('[merge-execution-readiness]',{authorisationId,error:error&&error.message?error.message:String(error)});}
  ...
  router.get('/api/os2/customer-merge-execution-readiness/:authorisationId',requirePermission('customer.merge.execution.authorise'),async(req,res)=>{
    ...
    try{
      const [[row]]=await pool.execute(`SELECT a.*,p.status plan_status,p.plan_hash current_plan_hash,p.current_snapshot_hash,
        ...
        (a.expires_at IS NOT NULL AND a.expires_at>NOW()) authorisation_unexpired
      ...
      const checks={
        ...
        authorisationUnexpired:Number(row.authorisation_unexpired)===1,
      ...
      logFailure(error,authorisationId);
