  ...
  router.get('/api/os2/customer-merge-execution-readiness/:authorisationId',requirePermission('customer.merge.execution.authorise'),async(req,res)=>{
    ...
    try{
      ...
      const checks={
        ...
        authorisationHasRequester:Boolean(row.requested_by)&&Boolean(row.requested_at),
        authorisationHasOwnerDecision:Boolean(row.authorised_by)&&Boolean(row.authorised_at),
        separationOfDuties:Number(row.requested_by)!==Number(row.authorised_by),
        ...
        planRevalidatedBeforeAuthorisation:Boolean(row.revalidated_at)&&Boolean(row.authorised_at)&&new Date(row.revalidated_at).getTime()<=new Date(row.authorised_at).getTime(),
        ...
        backupVerifiedBeforeAuthorisation:Boolean(row.verified_at)&&Boolean(row.authorised_at)&&new Date(row.verified_at).getTime()<=new Date(row.authorised_at).getTime(),
        ...
        restoreCompletedBeforeAuthorisation:Boolean(row.restore_completed_at)&&Boolean(row.authorised_at)&&new Date(row.restore_completed_at).getTime()<=new Date(row.authorised_at).getTime(),
