...
requireAll(migration025, ['ADD COLUMN restore_test_id BIGINT NULL', 'idx_merge_execution_restore', 'ORDER BY rt.completed_at DESC,rt.id DESC'], 'restore pin migration');
...
requireAll(previewReadiness, ["process.env.DB_NAME !== 'kloka_talk2me'", 'preview-data-verification.js', 'migrationLedgerBootstrapEvidenceRequired: true', 'migrationEvidenceVerifiedBeforeDatabaseConnection: true', 'migrationCompletionRequiresConfirmedLockRelease: true', '20260801_025_merge_authorisation_restore_pin.sql'], 'preview readiness');
...
requireAll(uatGate, ['migrations/20260801_025_merge_authorisation_restore_pin.sql', 'ADD COLUMN restore_test_id BIGINT NULL', 'preview-data-verification.js', "pkg.scripts?.[name] === command", 'previewDataVerificationRequired: true', 'mergeExecutionEnabled: false'], 'UAT gate');
requireAll(releaseCandidateGate, ["const restorePinMigration = '20260801_025_merge_authorisation_restore_pin.sql'", 'preview-data-verification.js', "'verify:preview-data'", "previewDataVerificationOrder: ['schema-verification.js', 'merge-restore-evidence-verification.js']", 'previewDataVerificationRequired: true', 'migrationCompletionRequiresConfirmedLockRelease: true', 'migrationConnectionClosedBeforeSuccess: true', 'mergeExecutionEnabled: false', 'package-lock.json is required before release-candidate freeze'], 'release candidate gate');
requireAll(releaseManifestCheck, ["pkg.version !== '0.60.0'", "'previewDataVerificationRequired: true'", "'requireExactArray(manifest.previewDataVerificationOrder'", "'exactPreviewDataVerificationOrderVerified: true'", "'exactRestorePinMigrationVerified: true'", "'manifestMigrationCountVerified: true'", "'releaseSourceCountsReconciled: true'", "'node release-manifest-check.js'", 'mergeExecutionEnabled: false'], 'release manifest governance');
...
console.log(JSON.stringify({ok:true,check:'schema-source-consistency',version:'0.60.0',exactMigrationInventory:25,previewDataVerificationRequired:true,previewDataVerificationOrder:['schema-verification.js','merge-restore-evidence-verification.js'],boundedVerifierOutputRequired:true,mergeExecutionEnabled:false},null,2));
