...
const pinMigration=read('migrations/20260801_025_merge_authorisation_restore_pin.sql');
...
requireText(routes,"requesterRecorded",'request evidence check');
requireText(routes,"authoriserRecorded",'owner decision evidence check');
...
requireText(routes,"LEFT JOIN os2_restore_tests rt ON rt.id=a.restore_test_id",'exact pinned restore join');
requireText(routes,"restoreEvidencePinned",'pinned restore evidence check');
requireText(routes,"restoreBelongsToBackup",'restore ownership check');
...
forbidText(routes,"ORDER BY rt2.completed_at",'latest restore substitution');
...
requireText(pinMigration,"restore_test_id",'restore pin migration');
