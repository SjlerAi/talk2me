...
module.exports = function createImportRouter({ pool, requireAuth, requestIp }) {
  ...
  function mappedRow(row,map){ const out={}; for(const field of allowedFields){ const source=map[field]; if(!source) continue; const raw=row[source]; out[field]=field.includes('date') ? dateValue(raw) : field==='monthly_invoice_amount' ? numberValue(raw) : text(raw, field==='email'?320:255); } out.cell_number_normalised=normalisePhone(out.cell_number); return out; }
  ...
  router.post('/api/imports/:token/analyse', requireAuth, ownerOnly, async (req,res) => {
    ...
    try {
      ...
      const duplicateKeys=new Map(); const errors=[];
      for(const item of mapped){ const d=item.data; if(!d.client_name){errors.push({row:item.rowNumber,error:'Missing client name'});continue;} if(!d.account_number&&!d.cell_number_normalised){errors.push({row:item.rowNumber,error:'Missing account number and cell number'});continue;} const key=`${d.account_number||''}|${d.cell_number_normalised||''}`; duplicateKeys.set(key,(duplicateKeys.get(key)||0)+1); }
      const valid=mapped.filter(item=>!errors.some(e=>e.row===item.rowNumber));
      let inserts=0,updates=0,unchanged=0,duplicates=0;
      const previewRows=[];
      for(const item of valid){ const d=item.data; const key=`${d.account_number||''}|${d.cell_number_normalised||''}`; if(duplicateKeys.get(key)>1) duplicates++;
        const [[existing]]=await pool.execute(`SELECT id,client_name,account_number,cell_number,email,city_town,package_name,handset,next_upgrade_date FROM clients WHERE is_active=1 AND ((:account<>'' AND account_number=:account) OR (:phone<>'' AND cell_number_normalised=:phone)) ORDER BY id LIMIT 1`,{account:d.account_number||'',phone:d.cell_number_normalised||''});
        let action='insert'; if(existing){ action=allowedFields.some(f=>d[f]!==undefined&&d[f]!==null&&String(existing[f]??'')!==String(d[f]??''))?'update':'unchanged'; }
        if(action==='insert')inserts++; else if(action==='update')updates++; else unchanged++;
        if(previewRows.length<200) previewRows.push({row:item.rowNumber,action,clientId:existing?.id||null,clientName:d.client_name,accountNumber:d.account_number,cellNumber:d.cell_number,warnings:duplicateKeys.get(key)>1?['Duplicate key in file']:[]});
      }
      preview.mapping=mapping; preview.analysis={mapped,errors,summary:{rows:preview.rows.length,inserts,updates,unchanged,duplicates,errors:errors.length},previewRows};
  ...
  router.post('/api/imports/:token/commit', requireAuth, ownerOnly, async (req,res) => {
    ...
    try {
      await connection.beginTransaction(); let inserted=0,updated=0,skipped=0;
      for(const item of preview.analysis.mapped){ if(preview.analysis.errors.some(e=>e.row===item.rowNumber)){skipped++;continue;} const d=item.data;
        const [[existing]]=await connection.execute(`SELECT id FROM clients WHERE is_active=1 AND ((:account<>'' AND account_number=:account) OR (:phone<>'' AND cell_number_normalised=:phone)) ORDER BY id LIMIT 1 FOR UPDATE`,{account:d.account_number||'',phone:d.cell_number_normalised||''});
        if(existing){ const sets=[]; const params={id:existing.id}; for(const field of allowedFields){ if(d[field]!==undefined&&d[field]!==null){sets.push(`\`${field}\`=:${field}`);params[field]=d[field];} } if(d.cell_number_normalised){sets.push('cell_number_normalised=:cell_number_normalised');params.cell_number_normalised=d.cell_number_normalised;} if(sets.length){sets.push('updated_at=NOW()');await connection.execute(`UPDATE clients SET ${sets.join(',')} WHERE id=:id`,params);updated++;} else skipped++;
        } else { const fields=['client_name','account_number','cell_number','cell_number_normalised','email','city_town','id_number','package_name','handset','previous_upgrade_date','next_upgrade_date','monthly_invoice_amount','cancellation_date','main_contact_name','main_contact_number']; const present=fields.filter(f=>d[f]!==undefined); const params={}; present.forEach(f=>params[f]=d[f]); await connection.execute(`INSERT INTO clients (${present.map(f=>`\`${f}\``).join(',')},is_active,created_at,updated_at) VALUES (${present.map(f=>`:${f}`).join(',')},1,NOW(),NOW())`,params); inserted++; }
      }
      await audit(connection,req,'monthly_import_committed',`Monthly import committed: ${preview.filename}`,{filename:preview.filename,inserted,updated,skipped,errors:preview.analysis.errors.length});
      await connection.commit(); previews.delete(req.params.token); res.json({ok:true,inserted,updated,skipped,errors:preview.analysis.errors.length});
