...
function fail(message) { console.error(`SCHEMA VERIFICATION FAILED: ${message}`); process.exitCode = 1; }

async function main() {
  const dbName=String(process.env.DB_NAME || '');
  if(dbName!=='kloka_talk2me') throw new Error('REFUSING_NON_PREVIEW_DATABASE');
  const pool=mysql.createPool({host:process.env.DB_HOST,port:Number(process.env.DB_PORT||3306),user:process.env.DB_USER,password:process.env.DB_PASSWORD||'',database:dbName,connectionLimit:2,namedPlaceholders:true,charset:'utf8mb4'});
  try {
    ...
    if(!process.exitCode) console.log(JSON.stringify({ok:true,database:dbName,requiredTables:REQUIRED_TABLES.length,verifiedColumnGroups:Object.keys(REQUIRED_COLUMNS).length,appliedMigrations:migrationRows.length,duplicateAccounts:0,duplicateMobiles:0},null,2));
  } finally { await pool.end(); }
}
main().catch(error=>{console.error(`SCHEMA VERIFICATION FAILED: ${error.message}`);process.exit(1);});
