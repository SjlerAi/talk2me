...
async function requireRecoverablePreviewBackup(connection,backupRunId,restoreTestId=null){
  ...
  const restoreSql=restoreTestId
    ?`SELECT id,status,completed_at,target_environment,expected_database_name,actual_database_name,failed_checks,backup_run_id FROM os2_restore_tests WHERE id=:restoreTestId AND backup_run_id=:backupRunId FOR UPDATE`
    :`SELECT id,status,completed_at,target_environment,expected_database_name,actual_database_name,failed_checks,backup_run_id FROM os2_restore_tests WHERE backup_run_id=:backupRunId ORDER BY completed_at DESC,id DESC LIMIT 1 FOR UPDATE`;
  const [[restore]]=await connection.execute(restoreSql,{restoreTestId,backupRunId});
  if(restoreTestId&&!restore)throw Object.assign(new Error('PINNED_RESTORE_TEST_REQUIRED'),{statusCode:409});
  const validRestore=restore&&Number(restore.backup_run_id)===Number(backupRunId)&&restore.status==='passed'&&restore.completed_at&&restore.target_environment==='isolated_preview_restore'&&restore.expected_database_name==='kloka_talk2me'&&restore.actual_database_name==='kloka_talk2me'&&Number(restore.failed_checks)===0;
    ...
    try{
      const result=await withTransaction(pool,async connection=>{
        ...
        await connection.execute(`INSERT INTO os2_customer_merge_execution_authorisations
          (merge_plan_id,plan_hash,snapshot_hash,backup_run_id,restore_test_id,change_reference,status,requested_by,requested_at,created_at,updated_at)
          VALUES(:mergePlanId,:planHash,:snapshotHash,:backupRunId,:restoreTestId,:changeReference,'pending',:actor,NOW(),NOW(),NOW())
          ON DUPLICATE KEY UPDATE plan_hash=VALUES(plan_hash),snapshot_hash=VALUES(snapshot_hash),backup_run_id=VALUES(backup_run_id),restore_test_id=VALUES(restore_test_id),change_reference=VALUES(change_reference),status='pending',requested_by=VALUES(requested_by),requested_at=NOW(),authorised_by=NULL,authorised_at=NULL,expires_at=NULL,revoked_by=NULL,revoked_at=NULL,revocation_reason=NULL,updated_at=NOW()`,{mergePlanId,planHash,snapshotHash,backupRunId,restoreTestId:evidence.restore.id,changeReference,actor:req.user.id});
        const [[saved]]=await connection.execute('SELECT id,restore_test_id FROM os2_customer_merge_execution_authorisations WHERE merge_plan_id=:mergePlanId',{mergePlanId});
        const details={mergePlanId,backupRunId,restoreTestId:saved.restore_test_id,planHash,snapshotHash,changeReference,replacesAuthorisationStatus:existing?existing.status:null};
        ...
        return{authorisationId:Number(saved.id),status:'pending',restoreTestId:Number(saved.restore_test_id),executionAvailable:false};
      });
  ...
  router.post('/api/os2/customer-merge-execution-authorisations/:id/decision',requirePermission('customer.merge.execution.authorise'),async(req,res)=>{
    ...
    try{
      const result=await withTransaction(pool,async connection=>{
        ...
        if(decision==='authorised'&&!positiveId(authorisation.restore_test_id))throw Object.assign(new Error('PINNED_RESTORE_TEST_REQUIRED'),{statusCode:409});
        const evidence=decision==='authorised'?await requireRecoverablePreviewBackup(connection,authorisation.backup_run_id,authorisation.restore_test_id):null;
        ...
        const details={expiresAt,isUnexpired:decision==='authorised',backupRunId:authorisation.backup_run_id,restoreTestId:evidence?evidence.restore.id:authorisation.restore_test_id,executionAvailable:false};
        ...
        return{authorisationId:id,status:decision,expiresAt,restoreTestId:authorisation.restore_test_id,executionAvailable:false};
      });
