...
module.exports=function createMergeExecutionAuthorisationRouter({pool,requireAuth}){
  ...
  router.post('/api/os2/customer-merge-execution-authorisations',requirePermission('customer.merge.execution.request'),async(req,res)=>{
    ...
    try{
      const result=await withTransaction(pool,async connection=>{
        ...
        const details={mergePlanId,backupRunId,restoreTestId:evidence.restore.id,planHash,snapshotHash,changeReference};
        ...
        await appendAudit(connection,{actorStaffId:req.user.id,actionType:'customer_merge_execution_authorisation_requested',entityType:'os2_customer_merge_execution_authorisations',entityId:saved.id,masterCustomerId:plan.survivor_customer_id,description:`Requested merge execution authorisation ${saved.id}; execution remains disabled`,after:{mergePlanId,backupRunId,restoreTestId:evidence.restore.id,changeReference},requestContext:context(req)});
  ...
  router.post('/api/os2/customer-merge-execution-authorisations/:id/decision',requirePermission('customer.merge.execution.authorise'),async(req,res)=>{
    ...
    try{
      const result=await withTransaction(pool,async connection=>{
        ...
        const details={expiresAt,isUnexpired:decision==='authorised'?Number(updated.is_unexpired)===1:false,backupRunId:authorisation.backup_run_id,restoreTestId:evidence?evidence.restore.id:null,executionAvailable:false};
  ...
  router.post('/api/os2/customer-merge-execution-authorisations/:id/revoke',requirePermission('customer.merge.execution.authorise'),async(req,res)=>{
    ...
    try{const result=await withTransaction(pool,async connection=>{const [[authorisation]]=await connection.execute('SELECT * FROM os2_customer_merge_execution_authorisations WHERE id=:id FOR UPDATE',{id});if(!authorisation)throw Object.assign(new Error('MERGE_EXECUTION_AUTHORISATION_NOT_FOUND'),{statusCode:404});if(authorisation.status!=='authorised'||authorisation.consumed_at)throw Object.assign(new Error('AUTHORISATION_NOT_REVOCABLE'),{statusCode:409});await connection.execute(`UPDATE os2_customer_merge_execution_authorisations SET status='revoked',revoked_by=:actor,revoked_at=NOW(),revocation_reason=:reason,updated_at=NOW() WHERE id=:id`,{id,actor:req.user.id,reason});await connection.execute(`INSERT INTO os2_customer_merge_execution_authorisation_history(authorisation_id,event_type,from_status,to_status,reason,changed_by,created_at) VALUES(:id,'revoked','authorised','revoked',:reason,:actor,NOW())`,{id,reason,actor:req.user.id});return{authorisationId:id,status:'revoked',executionAvailable:false};});res.json({ok:true,...result});}catch(error){res.status(error.statusCode||500).json({ok:false,error:error.statusCode?error.message:'MERGE_EXECUTION_AUTHORISATION_REVOKE_FAILED'});}
  });
