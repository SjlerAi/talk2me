...
requireMarkers(verifier, [
  ...
  'dependencyLockArtifactGovernanceProtected: files.some', 'dependencyLockGenerationRunbookProtected: files.some',
  'dependencyLockWorkflowRunbookProtected: files.some', 'dependencyLockArtifactRunbookProtected: files.some',
  'dependencyLockWorkflowProtected: files.some', 'activationGovernanceProtected: files.some',
  ...
  'recoveryReleaseGateProtected: files.some', 'recoveryRunbookProtected: files.some', 'duplicatePathsRejected: true',
  'secureDescriptorReads: true', 'pathAndDescriptorMetadataBound: true', 'exactReadByteCountRequired: true',
...
requireMarkers(preflight, [
  ...
  'dependencyLockArtifactGovernanceVerified: true', 'packageLockRequired: true', 'workspaceSourceIntegrityVerified: true',
...
requireMarkers(runbook, [
  'Dependency lock generation', 'Dependency lock artifact verification', 'package-lock.json',
  'dependency-lock-generator-check.js', 'dependency-lock-workflow-check.js', 'dependency-lock-artifact-check.js',
  'DEPENDENCY_LOCK_WORKFLOW_RUNBOOK.md', 'DEPENDENCY_LOCK_ARTIFACT_REVIEW_RUNBOOK.md',
...
if (pkg.scripts['verify:dependency-lock-artifact'] !== 'node dependency-lock-artifact-verification.js') throw new Error('Missing verify:dependency-lock-artifact command');
if (pkg.scripts['check:dependency-lock-artifact'] !== 'node dependency-lock-artifact-check.js') throw new Error('Missing check:dependency-lock-artifact command');
for (const marker of ['node --check workspace-source-integrity.js','node --check workspace-source-integrity-check.js','node --check dependency-lock-artifact-verification.js','node --check dependency-lock-artifact-check.js','node dependency-lock-artifact-check.js','node workspace-source-integrity-check.js']) {
...
console.log(JSON.stringify({
  ...
  artifactVerifierPackageCommandRequired: true,
  artifactGovernancePackageCommandRequired: true,
  artifactVerifierSyntaxValidationRequired: true,
  artifactGovernanceNormalExecutionRequired: true,
