  ...
  router.post('/api/os2/customer-merge-plans/prepare',requirePermission('customer.merge.plan'),async(req,res)=>{
    ...
    try{
      const result=await withTransaction(pool,async connection=>{
        ...
        const [insert]=await connection.execute(`INSERT INTO os2_customer_merge_plans(duplicate_case_id,survivor_customer_id,source_customer_id,status,plan_json,plan_hash,blocker_count,conflict_count,prepared_by,prepared_at,created_at,updated_at)
          VALUES(:caseId,:survivorId,:sourceId,'draft',:plan,:planHash,:blockers,:conflicts,:actor,NOW(),NOW(),NOW())
          ON DUPLICATE KEY UPDATE survivor_customer_id=VALUES(survivor_customer_id),source_customer_id=VALUES(source_customer_id),status='draft',plan_json=VALUES(plan_json),plan_hash=VALUES(plan_hash),blocker_count=VALUES(blocker_count),conflict_count=VALUES(conflict_count),prepared_by=VALUES(prepared_by),prepared_at=NOW(),approved_by=NULL,approved_at=NULL,rejected_by=NULL,rejected_at=NULL,decision_reason=NULL,updated_at=NOW()`,{caseId,survivorId,sourceId,plan:JSON.stringify(plan),planHash,blockers:plan.blockerCount,conflicts:plan.conflictCount,actor:req.user.id});
        const [[saved]]=await connection.execute('SELECT id FROM os2_customer_merge_plans WHERE duplicate_case_id=:caseId',{caseId});
        await connection.execute(`INSERT INTO os2_customer_merge_plan_history(merge_plan_id,event_type,to_status,reason,details_json,changed_by,created_at) VALUES(:id,'prepared','draft','Merge plan prepared',:details,:actor,NOW())`,{id:saved.id,details:JSON.stringify({planHash,blockerCount:plan.blockerCount,conflictCount:plan.conflictCount}),actor:req.user.id});
        await appendAudit(connection,{actorStaffId:req.user.id,actionType:'customer_merge_plan_prepared',entityType:'os2_customer_merge_plans',entityId:saved.id,masterCustomerId:survivorId,description:`Prepared merge plan ${saved.id}; execution disabled`,after:{caseId,survivorId,sourceId,planHash,blockerCount:plan.blockerCount},requestContext:context(req)});
        return {mergePlanId:Number(saved.id),planHash,plan};
      });
  ...
  router.post('/api/os2/customer-merge-plans/:planId/decision',requirePermission('customer.merge.approve'),async(req,res)=>{
    ...
    try{
      const result=await withTransaction(pool,async connection=>{
        ...
        if(decision==='approved'&&Number(plan.blocker_count)>0)throw Object.assign(new Error('MERGE_PLAN_HAS_BLOCKERS'),{statusCode:409});
        await connection.execute(`UPDATE os2_customer_merge_plans SET status=:decision,approved_by=IF(:decision='approved',:actor,NULL),approved_at=IF(:decision='approved',NOW(),NULL),rejected_by=IF(:decision='rejected',:actor,NULL),rejected_at=IF(:decision='rejected',NOW(),NULL),decision_reason=:reason,updated_at=NOW() WHERE id=:id`,{id:planId,decision,actor:req.user.id,reason});
        await connection.execute(`INSERT INTO os2_customer_merge_plan_history(merge_plan_id,event_type,from_status,to_status,reason,details_json,changed_by,created_at) VALUES(:id,'decision','draft',:decision,:reason,:details,:actor,NOW())`,{id:planId,decision,reason,details:JSON.stringify({planHash:expectedHash,executionAvailable:false}),actor:req.user.id});
        await appendAudit(connection,{actorStaffId:req.user.id,actionType:'customer_merge_plan_decided',entityType:'os2_customer_merge_plans',entityId:planId,masterCustomerId:plan.survivor_customer_id,description:`Merge plan ${planId} ${decision}; execution disabled`,before:{status:plan.status},after:{status:decision,reason,planHash:expectedHash},requestContext:context(req)});
        return {mergePlanId:planId,status:decision,executionAvailable:false};
      });
