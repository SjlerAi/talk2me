...
class MySqlSessionStore extends session.Store {
  ...
  get(sid, callback) {
    db.execute(`SELECT session_data FROM app_sessions WHERE session_id=:id AND expires_at>NOW() LIMIT 1`, { id:this.key(sid) })
      .then(([rows]) => {
        ...
        try { callback(null, JSON.parse(rows[0].session_data)); }
  ...
  destroy(sid, callback=()=>{}) {
    db.execute('DELETE FROM app_sessions WHERE session_id=:id',{id:this.key(sid)})
      .then(()=>callback(null)).catch(callback);
  }

  touch(sid, value, callback=()=>{}) {
    // Deliberately do not extend expiry: every login lasts at most eight hours.
    callback(null);
  }
}
...
app.use(session({
  name: 'talk2me.sid',
  secret: process.env.SESSION_SECRET || 'talk2me-dev-secret-change-me',
...
app.use((req, res, next) => {
  ...
  // The deployed package is the authoritative release. A stale cPanel APP_VERSION
  // must never make a newer deployment appear to be an older version.
