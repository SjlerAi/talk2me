(() => {
  ...
  function ensure(){
    ...
    byId('oppNext').onclick=()=>{const pages=Math.max(1,Math.ceil(filteredGroups().length/pageSize));if(page<pages){page+=1;render();window.scrollTo({top:view.offsetTop-20,behavior:'smooth'});}};
    return view;
  }
  ...
  function normalise(v){return String(v||'').normalize('NFKC').toLowerCase().replace(/[^a-z0-9]/g,'');}
  function groupItems(){
    const groups=new Map();
    for(const item of items){
      const nameKey=normalise(item.client_name)||`id${item.id}`;
      const contactKey=normalise(item.cell_number)||normalise(item.email);
      const key=contactKey?`${nameKey}|${contactKey}`:nameKey;
      if(!groups.has(key))groups.set(key,{primary:item,records:[],search:''});
      groups.get(key).records.push(item);
    }
    return [...groups.values()].map(group=>{
      group.search=group.records.flatMap(i=>[i.client_name,i.account_number,i.cell_number,i.email,i.city_town,i.package_name,i.handset,i.assigned_staff]).join(' ').toLowerCase();
      return group;
    });
  }
  function filteredGroups(){const groups=groupItems();return query?groups.filter(g=>g.search.includes(query)):groups;}
  function recordDate(i){return type==='birthdays'?i.birthday:type==='renewals'?i.cancellation_date:i.next_upgrade_date;}
  function render(){
    ...
    byId('oppPageInfo').textContent=filtered.length?`Showing ${start+1}-${Math.min(start+pageSize,filtered.length)} of ${filtered.length} customers`:'0 customers';
    ...
    list.innerHTML=visible.map(group=>{
      const i=group.primary;
      const wa=String(i.cell_number||'').replace(/\D/g,'');
      const records=group.records.map(r=>`<div class="opp-record"><strong>${esc(r.account_number||'No account')}</strong><span>${esc([r.package_name,r.handset].filter(Boolean).join(' · ')||'No package details')}</span><span>${esc(fmt(recordDate(r)))}</span></div>`).join('');
      return `<article class="opp-card"><div><div class="opp-title-row"><strong>${esc(i.client_name)}</strong>${group.records.length>1?`<span class="opp-count">${group.records.length} records</span>`:''}</div><div class="opp-meta"><span>${esc(i.assigned_staff)}</span><span>${esc(type)}</span></div><div class="opp-records">${records}</div></div><div class="opp-actions"><button data-client="${i.id}">Customer</button>${i.cell_number?`<a href="tel:${esc(i.cell_number)}">Call</a>`:''}${wa?`<a href="https://wa.me/${esc(wa)}" target="_blank">WhatsApp</a>`:''}<button data-contacted="${i.id}">Mark contacted</button><button class="primary" data-followup="${i.id}">Follow-up</button></div></article>`;
    }).join('');
  ...
  async function load(){ensure();byId('oppStatus').textContent='Loading opportunities...';try{const r=await api(`/api/opportunities?type=${encodeURIComponent(type)}&days=${days}`);const d=await r.json();if(!r.ok||!d.ok)throw new Error(d.error||'OPPORTUNITY_QUERY_FAILED');items=d.items||[];const groups=groupItems();byId('oppStatus').textContent=`${groups.length} customer${groups.length===1?'':'s'} · ${items.length} qualifying record${items.length===1?'':'s'} · ${d.teamView?'Team view':'Your assigned clients'}`;render();}catch(e){byId('oppStatus').textContent=`Could not load opportunities: ${e.message}`;}}
