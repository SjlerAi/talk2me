...
async function loadDashboard(){
  ...
  try{
    const response=await fetch('/api/dashboard',{headers:{Accept:'application/json'}}); const data=await response.json(); if(!response.ok||!data.ok) throw new Error(data.error||'Dashboard could not load');
    ...
    byId('metricClockedIn').textContent=`${m.clockedIn}/${m.activeStaff}`; byId('activeStaffText').textContent=`${m.activeStaff} active staff accounts`; byId('workBadge').textContent=m.overdue; byId('alertBadge').textContent=m.approvals; byId('systemMessage').textContent='Live test data loaded from kloka_talk2me.';
  ...
  }catch(error){ byId('systemMessage').textContent=`Database connection not ready: ${error.message}`; byId('activityRows').innerHTML='<tr><td colspan="5">Live data is not available yet.</td></tr>'; toast('Database connection needs configuration'); }
}
...
async function openCustomer(id){
  ...
  try{
    const response=await fetch(`/api/customers/${encodeURIComponent(id)}`); const data=await response.json(); if(!response.ok||!data.ok) throw new Error(data.error||'Customer could not load');
  ...
  }catch(error){ byId('customerName').textContent='Customer unavailable'; byId('customerSummary').textContent=error.message; toast('Customer 360 could not load'); }
}
...
search.addEventListener('input',()=>{
  ...
  searchTimer=setTimeout(async()=>{try{
    const response=await fetch(`/api/customers/search?q=${encodeURIComponent(value)}`); const data=await response.json(); if(!response.ok||!data.ok) throw new Error(data.error||'Search failed');
  ...
  }catch(error){results.innerHTML=`<div class="result"><b>Search unavailable</b><span>${escapeHtml(error.message)}</span></div>`}},300);
});
