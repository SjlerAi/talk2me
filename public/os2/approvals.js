(() => {
  ...
  function ensureStyles(){
    ...
    style.textContent='.approval-head{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:22px}.approval-head h2{margin:4px 0}.approval-list{display:grid;gap:14px}.approval-card{background:#fff;border:1px solid var(--line);border-radius:16px;padding:18px;display:grid;grid-template-columns:1fr auto;gap:16px;box-shadow:0 10px 28px rgba(46,93,119,.07)}.approval-meta{display:flex;gap:12px;flex-wrap:wrap;color:var(--muted);font-size:12px;margin:8px 0}.approval-actions{display:flex;gap:8px;align-items:center}.approval-actions button{height:40px;padding:0 14px;border-radius:10px;border:1px solid var(--line);background:#fff;font-weight:800;cursor:pointer}.approval-actions .approve{background:var(--green);color:#fff;border-color:var(--green)}.approval-actions .reject{background:var(--red);color:#fff;border-color:var(--red)}.approval-empty{padding:32px;text-align:center;color:var(--muted)}@media(max-width:760px){.approval-card{grid-template-columns:1fr}.approval-actions{justify-content:flex-start}}';
    document.head.appendChild(style);
  }

  function approvalMarkup(){
    return '<section class="admin-card"><div class="approval-head"><div><span>MANAGEMENT CONTROL</span><h2>Approvals</h2><p id="approvalStatus">Loading pending claims...</p></div><button class="secondary" id="refreshApprovals">Refresh</button></div><div class="approval-list" id="approvalList"></div></section>';
  }

  function getContainer(){
    ensureStyles();
    const adminBody=byId('adminBody');
    const adminView=byId('administrationView');
    if(adminBody&&adminView&&!adminView.hidden){
      adminBody.innerHTML=approvalMarkup();
      return adminBody;
    }
    if(!standaloneView){
      standaloneView=document.createElement('section');
      standaloneView.className='content';
      standaloneView.id='approvalView';
      standaloneView.hidden=true;
      document.querySelector('main').appendChild(standaloneView);
    }
    document.querySelectorAll('main > .content').forEach(section=>{if(section!==standaloneView)section.hidden=true;});
    standaloneView.hidden=false;
    standaloneView.innerHTML=approvalMarkup();
    return standaloneView;
  }

  async function load(){
    const container=getContainer();
    const refresh=byId('refreshApprovals');
    ...
    try{
      ...
      byId('approvalList').querySelectorAll('[data-customer]').forEach(btn=>btn.addEventListener('click',()=>{
        const id=Number(btn.dataset.customer);
        if(id&&typeof window.openCustomer==='function')window.openCustomer(id);
      }));
      byId('approvalList').querySelectorAll('[data-decision]').forEach(btn=>btn.addEventListener('click',()=>decide(btn)));
    }catch(error){
      byId('approvalStatus').textContent=`Could not load approvals: ${error.message}`;
      if(container)container.querySelector('#approvalList').innerHTML='';
    }
  }
  ...
  async function decide(button){
    ...
    try{
      ...
      const data=await r.json();
      if(!r.ok||!data.ok)throw new Error(data.error||'APPROVAL_DECISION_FAILED');
      ...
      const messages={SELF_APPROVAL_NOT_ALLOWED:'You cannot approve your own claim.',REQUEST_ALREADY_DECIDED:'This request has already been decided.'};
      notify(messages[error.message]||`Decision failed: ${error.message}`);
