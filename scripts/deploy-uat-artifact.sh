...
APP_DIR=/home/uent/talk2me.uent.co.za
BACKUP_DIR=/home/uent/deployment-backups/talk2me
RELEASES_DIR=/home/uent/releases/talk2me
...
if (value.schema !== 1 || value.service !== 'talk2me-crm' || value.repository !== 'SjlerAi/talk2me' || value.environment !== 'uat' || value.commit !== expectedCommit || String(value.workflowRunId) !== String(expectedRun)) process.exit(1);
NODE
...
rollback() {
  ...
  if [ "$ACTIVATION_STARTED" = true ]; then
    echo "Activation failed; restoring the previous application release." >&2
    for target in "${targets[@]}"; do rm -rf "$APP_DIR/$target"; done
    tar -xzf "$BACKUP_FILE" -C "$APP_DIR"
    if [ "$NODE_MODULES_REPLACED" = true ]; then
      rm -rf "$APP_DIR/node_modules"
    fi
    if [ "$NODE_MODULES_MOVED" = true ]; then
      mv "$OLD_NODE_MODULES" "$APP_DIR/node_modules"
    fi
    mkdir -p "$APP_DIR/tmp"
...
touch "$APP_DIR/tmp/restart.txt"
sleep 8
curl -LfsS --connect-timeout 10 --max-time 25 "$SITE_URL/api/health" | "$NODE_BIN" -e "let s='';process.stdin.on('data',d=>s+=d).on('end',()=>{const v=JSON.parse(s);if(v.status!=='ok')process.exit(1)})"
curl -LfsS --connect-timeout 10 --max-time 25 "$SITE_URL/api/release" | EXPECTED_COMMIT="$EXPECTED_COMMIT" EXPECTED_RUN_ID="$EXPECTED_RUN_ID" "$NODE_BIN" -e "let s='';process.stdin.on('data',d=>s+=d).on('end',()=>{const v=JSON.parse(s);if(v.commit!==process.env.EXPECTED_COMMIT||String(v.workflowRunId)!==process.env.EXPECTED_RUN_ID)process.exit(1)})"

trap - ERR
rm -f "$ARTIFACT_PATH" "$CHECKSUM_PATH"
find "$BACKUP_DIR" -maxdepth 1 -type f -name 'talk2me-before-*.tar.gz' -printf '%T@ %p\n' | sort -nr | tail -n +6 | cut -d' ' -f2- | xargs -r rm -f
...
echo "Activated Talk2Me release $EXPECTED_COMMIT from workflow $EXPECTED_RUN_ID."
